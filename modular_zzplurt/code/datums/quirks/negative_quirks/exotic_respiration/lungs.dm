...
/obj/item/organ/lungs/exotic/on_mob_insert(mob/living/carbon/receiver, special = FALSE, movement_flags)
	. = ..()
	receiver.clear_alert(ALERT_NOT_ENOUGH_BZ)

// BZ is exchanged with CO2 just like oxygen
/obj/item/organ/lungs/exotic/proc/breathe_bz(mob/living/carbon/breather, datum/gas_mixture/breath, bz_pp, old_bz_pp)
	if(bz_pp < safe_bz_min && !HAS_TRAIT(breather, TRAIT_NO_BREATHLESS_DAMAGE))
		...
		var/gas_breathed = handle_suffocation(breather, bz_pp, safe_bz_min, breath.moles[/datum/gas/bz])
		if(bz_pp)
...
/obj/item/organ/lungs/exotic/proc/breathe_n2o(mob/living/carbon/breather, datum/gas_mixture/breath, n2o_pp, old_n2o_pp)
	if(n2o_pp < safe_n2o_min && !HAS_TRAIT(breather, TRAIT_NO_BREATHLESS_DAMAGE))
		...
		var/gas_breathed = handle_suffocation(breather, n2o_pp, safe_n2o_min, breath.moles[/datum/gas/nitrous_oxide])
		if(n2o_pp)
...
/obj/item/organ/lungs/exotic/proc/breathe_co2(mob/living/carbon/breather, datum/gas_mixture/breath, co2_pp, old_co2_pp)
	if(co2_pp < safe_co2_min && !HAS_TRAIT(breather, TRAIT_NO_BREATHLESS_DAMAGE))
		...
		var/gas_breathed = handle_suffocation(breather, co2_pp, safe_co2_min, breath.moles[/datum/gas/carbon_dioxide])
		if(co2_pp)
...
/obj/item/organ/lungs/exotic/plasma // ditto n2 but for plasmamen lungs
	safe_plasma_min = 16
	safe_plasma_max = 0
	breathgas = "plasma"
